...
function sandboxPolicy(context: RuntimeContext, mode: RuntimePermissionMode): TurnInput['sandboxPolicy'] {
  const access = { type: 'restricted' as const, includePlatformDefaults: true, readableRoots: context.effectivePolicy.readableRoots }
  if (mode === 'read-only') return { type: 'readOnly', access }
  return {
    type: 'workspaceWrite', writableRoots: context.effectivePolicy.writableRoots, readOnlyAccess: access,
...
function executionContext(context: RuntimeContext, mode: RuntimePermissionMode): ExecutionContext {
  ...
  return {
    ...
    turn: { cwd, approvalPolicy: approvalPolicy(mode), sandboxPolicy: sandboxPolicy(context, mode) },
  }
}
...
export class CodexRuntimeAdapter implements ConversationRuntimeAdapter {
  ...
  async getManifest(): Promise<RuntimeManifest> {
    return {
      provider: this.provider, runtimeVersion: 'cody-web-core/0.5.0', protocolVersion: WORKBENCH_RUNTIME_PROTOCOL_VERSION,
  ...
  private async ensureRuntime(context: RuntimeContext): Promise<CodexSessionManager> {
    ...
    this.host = createAppServerHost({ command: this.command(), cwd: context.workspacePath, ...(this.options.env ? { env: this.options.env } : {}), initializeParams: { clientInfo: { name: 'codywork', title: 'CodyWork', version: '0.5.0' }, capabilities: { experimentalApi: true, requestAttestation: false } } })
