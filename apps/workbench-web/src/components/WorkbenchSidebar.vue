  ...
  <aside :class="['sidebar', { collapsed }]">
    ...
    <div class="sidebar-scroll">
      <template v-if="workspace">
        ...
        <button class="sidebar-item workspace-search-entry" :class="{ active: workspaceConversationActive }" :aria-label="collapsed ? 'Workspace 会话' : undefined" :title="collapsed ? 'Workspace 会话' : undefined" @click="emit('openWorkspaceConversations')"><span>⌕</span><span class="sidebar-label"><strong>Workspace 会话</strong><small>直接使用 Codex</small></span><em v-if="workspaceConversationCount">{{ workspaceConversationCount }}</em></button>
...
const realtimeLabel = computed(() => {
  if (!props.realtimeActive) return 'Realtime standby'
...
const realtimeDetail = computed(() => {
  ...
  if (!props.realtimeActive) return '打开会话后建立 Browser WebSocket'
