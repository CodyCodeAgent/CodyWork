<template>
  <div class="channel-settings">
    <div class="channel-intro">
      ...
      <button v-if="!accounts.length" class="primary" type="button" :disabled="busy" @click="newAccount">＋ 配置机器人</button>
    </div>
    ...
    <div class="channel-layout">
      ...
      <main class="editor">
        ...
        <section v-if="diagnostics" class="diagnostics">
          ...
          <div class="runtime-facts"><span><small>最近连接</small>{{ formatTime(selected?.connectedAt) }}</span><span><small>最近事件</small>{{ formatTime(selected?.lastEventAt) }}</span><span><small>最近投递</small>{{ formatTime(selected?.lastDeliveryAt) }}</span><span><small>观察会话</small>{{ diagnostics.runtime.observedConversations }} / {{ diagnostics.bindings }}</span></div>
