...
const { state: conversationState, connect: connectConversationState, reset: resetConversationState, enqueueUserMessage, bindQueuedUserMessage, failQueuedUserMessage } = useConversationController()
...
async function sendMessage(): Promise<void> { if (!workspace.value || !selectedConversation.value || sending.value) return; const conversationId = selectedConversation.value.id; const content = draft.value.trim(); const turnSkills = [...selectedSkillsForTurn.value]; if (!composerHasContent({ text: content, skills: turnSkills })) return; const optimisticId = `local:${conversationId}:${Date.now().toString(36)}:${String(++nextOptimisticMessageId)}`; const skillReferences = turnSkills.flatMap((id) => { const skill = runtimeSkills.value.find((candidate) => candidate.id === id); return skill ? [{ name: skill.name, path: skill.id, displayName: skill.label }] : [] }); enqueueUserMessage({ id: optimisticId, text: content, ...(skillReferences.length ? { skills: skillReferences } : {}) }); draft.value = ''; draftByConversationId.delete(conversationId); lastSubmittedPromptByConversationId.set(conversationId, content); sending.value = true; try { const accepted = await api.sendMessage(workspace.value.id, conversationId, content, resolveComposerSubmitMode(isRunning.value, selectedSubmitMode.value), { ...(selectedModel.value ? { model: selectedModel.value } : {}), ...(selectedReasoning.value ? { reasoningEffort: selectedReasoning.value } : {}), collaborationMode: selectedCollaborationModeKind.value, ...(turnSkills.length ? { skills: turnSkills } : {}) }); bindQueuedUserMessage(optimisticId, accepted.turnId); selectedSkillsForTurn.value = [] } catch (cause) { const message = cause instanceof Error ? cause.message : String(cause); failQueuedUserMessage(optimisticId, message); draft.value = content; draftByConversationId.set(conversationId, content); selectedSkillsForTurn.value = turnSkills; lastSubmittedPromptByConversationId.delete(conversationId); error.value = message } finally { sending.value = false } }
